...
import { TEST_CITIES, TEST_COMPUTED } from "./test/fixtures";

const [cityA, cityB, cityC, cityD] = TEST_CITIES;
...
describe("getStats", () => {
  it("returns yearly sub-object for a valid year", () => {
    const s = getStats(cityA, "2020");
    expect(s).toBe(cityA.years["2020"]);
  });

  it('returns the city itself for "all"', () => {
    expect(getStats(cityA, "all")).toBe(cityA);
  });

  it("returns null for a missing year", () => {
    expect(getStats(cityA, "2019")).toBeNull();
  });

  it("returns null when years is empty", () => {
    expect(getStats(cityD, "2020")).toBeNull();
  });
});
...
describe("getValue", () => {
  it("price mode: returns correct median_sqm value", () => {
    expect(getValue(cityA, "price", "residential", "2020")).toBe(2000);
  });

  it("price mode: returns null when field is missing", () => {
    // cityB has no median_sqm_house in 2020
    expect(getValue(cityB, "price", "house", "2020")).toBeNull();
  });

  it("rent mode: returns correct rent field", () => {
    expect(getValue(cityA, "rent", "residential", "2020")).toBe(10);
  });

  it("rent mode + land filter: returns null (land has no rent)", () => {
    expect(getValue(cityA, "rent", "land", "2020")).toBeNull();
  });

  it("yield: computes (rent * 12 / price) * 100", () => {
    // cityA 2020: rent_residential=10, median_sqm=2000
    // yield = (10 * 12 / 2000) * 100 = 6.0
    expect(getValue(cityA, "yield", "residential", "2020")).toBe(6.0);
  });

  it("yield: returns null when price or rent missing", () => {
    // cityC 2022 has no rent_residential
    expect(getValue(cityC, "yield", "residential", "2022")).toBeNull();
  });

  it("yield: returns null for land filter (no rent field)", () => {
    expect(getValue(cityA, "yield", "land", "2020")).toBeNull();
  });

  it('year="all" uses aggregate data', () => {
    expect(getValue(cityA, "price", "residential", "all")).toBe(2300);
  });
});
...
describe("getChange", () => {
  it("price: correct pct, base, end", () => {
    // cityA: 2000 → 2600 = +30%
    const ch = getChange(cityA, "price", "residential", "2020", "2023");
  ...
  it("returns null when base year missing", () => {
    expect(getChange(cityB, "price", "residential", "2021", "2023")).toBeNull();
  });

  it("rent mode: correct structure", () => {
    // cityA: rent_residential 2020=10, 2023=13 → +30%
    const ch = getChange(cityA, "rent", "residential", "2020", "2023");
    expect(ch).not.toBeNull();
    if (!ch) {
      return;
    }
  ...
  it("yield mode: computes yield at both years and returns % change", () => {
    // cityA 2020: yield = (10*12/2000)*100 = 6.0
    // cityA 2023: yield = (13*12/2600)*100 = 6.0
    // pct = 0%
    const ch = getChange(cityA, "yield", "residential", "2020", "2023");
    expect(ch).not.toBeNull();
    if (!ch) {
      return;
    }
  ...
  it("yield: returns null when rent missing at one year", () => {
    // cityB has no 2021 data at all
    expect(getChange(cityB, "yield", "residential", "2020", "2021")).toBeNull();
  });

  it("returns null for rent mode with land filter", () => {
    expect(getChange(cityA, "rent", "land", "2020", "2023")).toBeNull();
  });
});
...
describe("changeToColor", () => {
  ...
  it("unknown key → empty string", () => {
    expect(
      changeToColor(10, "price", "residential", "1900", "1901", changeScales)
    ).toBe("");
  });
  ...
  it("symmetric hues for ± same magnitude", () => {
    ...
    const huePos = parseInt(pos.match(/hsl\((\d+)/)?.[1] ?? "0", 10);
    const hueNeg = parseInt(neg.match(/hsl\((\d+)/)?.[1] ?? "0", 10);
...
describe("computeFeatureState", () => {
  ...
  it("change mode + insufficient data → fillColor empty", () => {
    // cityC (99999) has count=10 in 2022, below MIN_SAMPLES_PER_YEAR=15
    const state = computeFeatureState(
      cityC,
      computed,
      "price",
